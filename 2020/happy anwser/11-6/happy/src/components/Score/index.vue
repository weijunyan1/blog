<template>
  <div class="score_container">
    ...
    <div class="your_scores_container">
      ...
      <div class="result_tip">{{length}}</div>
    </div>
...
//生命周期函数
  ...
  methods: {
      //判断存储id值和正确的id值是否相同
      arr(){
          this.answers.findIndex((item,index)=>{
              //如果相同分数增加
      ...
      err(){
          //根据分数写相应的评语
          if(this.num <= 20){
              this.length = this.scoreTipsArr[0]
          }else  if(this.num <= 40){
              this.length = this.scoreTipsArr[1]
          }else if(this.num <= 60){
              this.length = this.scoreTipsArr[2]
          }else if(this.num <= 80){
              this.length = this.scoreTipsArr[3]
          }else if(this.num <= 100){
              this.length = this.scoreTipsArr[4]
          }
      },
      ...
      again(){
          ...
          this.$router.push('/')
          //并修改题号清除数据
          this.$store.dispatch('againchange')
      }
  }
};
</script>
