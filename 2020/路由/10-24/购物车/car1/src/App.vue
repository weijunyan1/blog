  ...
  <div id="app">
    <div class="container">
      <div class="cart">
        <!-- //////////////////调用 -->
        <!-- 子组件 -->
        <HelloWorld :uname="uname"></HelloWorld>
        <bode :list="list" @change="chang"></bode>
        <foot :list="list"></foot>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods:{
    chang(val){
      let num=this.list.findIndex(item=>item.id==val.id);
      if(val.id==this.list[num].id){
        if(val.type=="cut"){
          this.list[num].num--
          console.log(111)
        }else if(val.type=="plas"){
          this.list[num].num++
          console.log(111)
